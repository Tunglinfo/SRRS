...
//create a canvas to grab an image for upload (uploading to our Object Detection API)
let imageCanvas = document.createElement('canvas');
let imageCtx = imageCanvas.getContext("2d");

//create a canvas for drawing object boundaries (displaying our boxes and labels)
...
function postFile(file) {
    ...
    let formdata = {};
    formdata.image = file;
    formdata.threshold = scoreThreshold;
    //formdata.append("image", file);
    //formdata.append("threshold", scoreThreshold);
    console.log('!!!');
    socket.emit('catch-frame', formdata);
}

socket.on('return-frame', function (msg) {
        let objects = JSON.parse(msg);
...
function startObjectDetection() {
    ...
    imageCtx.drawImage(v, 0, 0, v.videoWidth, v.videoHeight, 0, 0, uploadWidth, uploadWidth * (v.videoHeight / v.videoWidth));
    setInterval(()=>{imageCanvas.toBlob(postFile, 'image/jpeg')},1000/5);
}
